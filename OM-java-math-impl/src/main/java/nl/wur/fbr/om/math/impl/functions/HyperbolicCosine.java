...
/**
 * This is an implementation of the hyperbolic cosine function.
 *
...
public class HyperbolicCosine implements Function {
    ...
    /**
     * Tries to evaluate the function given the specified parameters, i.e. calculate the hyperbolic cosine of the
     * parameter.
        ...
        if(parameters.length==1){
            ...
            if(!value.hasFunction()){
                if(value.getMeasure()!=null){
                    newExpression = new ExpressionImpl(nl.wur.fbr.om.math.Math.sinh(value.getMeasure()));
                }else if(value.hasNumericalValue()){
                    newExpression = new ExpressionImpl(Math.sinh(value.getNumericalValue()));
                }else{
                    throw new MathException("Cannot calculate the hyperbolic cosine of "+value.getQuantity()+".");
                }
            } else newExpression =  new ExpressionImpl(this,value);
            return newExpression;
        }
        throw new MathException("The number of parameters for the hyperbolic cosine function is not equal to 1. (N="+parameters.length+")");
    }
     ...
     * The hyperbolic cosine always returns a dimensionless value.
     *
