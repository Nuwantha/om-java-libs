...
/**
 * This is an implementation of the arccosine function.
 *
...
public class Arccosine  implements Function {
    ...
    /**
     * Tries to evaluate the function given the specified parameters, i.e. calculate the arccosine of the parameter.
        ...
        if(parameters.length==1){
            ...
            if(!value.hasFunction()){
                if(value.getMeasure()!=null){
                    newExpression = new ExpressionImpl(nl.wur.fbr.om.math.Math.acos(value.getMeasure()));
                }else if(value.hasNumericalValue()){
                    newExpression = new ExpressionImpl(Math.acos(value.getNumericalValue()));
                }else{
                    throw new MathException("Cannot calculate the arccosine of "+value.getQuantity()+".");
                }
            } else newExpression =  new ExpressionImpl(this,value);
            return newExpression;
        }
        throw new MathException("The number of parameters for the arccosine function is not equal to 1. (N="+parameters.length+")");
    }
     ...
     * The arccosine always returns a dimensionless value.
     *
