...
/**
 * This is an implementation of a function that determines the dot product of two vector parameters.
 *
 * @author Don Willems on 09/10/15.
 */
public class DotProduct implements Function {

    /**
     * Tries to evaluate the function given the specified parameters, i.e. calculate the dot product of the two
     * parameter vectors.
        ...
        if(parameters.length==2){
            ...
            if(!x.hasFunction() && !y.hasFunction()){
                if(x.getMeasure()!=null && y.getMeasure()!=null){
                    newExpression = new ExpressionImpl(nl.wur.fbr.om.math.Math.dotProduct(x.getMeasure(), y.getMeasure()));
                }else{
                    throw new MathException("Cannot evaluate the dot product of "+x.getQuantity()+" and "+y.getQuantity()+".");
                }
     ...
     * The dot product has a dimension of the dimension of the parameter vectors (which have to be equal) squared.
     *
