...
/**
 * This is an implementation of the arcsine function.
 *
...
public class Arcsine implements Function {
    ...
    /**
     * Tries to evaluate the function given the specified parameters, i.e. calculate the arcsine of the parameter.
        ...
        if(parameters.length==1){
            ...
            if(!value.hasFunction()){
                if(value.getMeasure()!=null){
                    newExpression = new ExpressionImpl(nl.wur.fbr.om.math.Math.asin(value.getMeasure()));
                }else if(value.hasNumericalValue()){
                    newExpression = new ExpressionImpl(Math.asin(value.getNumericalValue()));
                }else{
                    throw new MathException("Cannot calculate the arcsine of "+value.getQuantity()+".");
                }
            } else newExpression =  new ExpressionImpl(this,value);
            return newExpression;
        }
        throw new MathException("The number of parameters for the arcsine function is not equal to 1. (N="+parameters.length+")");
    }
     ...
     * The arcsine always returns a dimensionless value.
     *
