...
/**
 * This is an implementation of the hyperbolic tangent function.
 *
...
public class HyperbolicTangent implements Function {
    ...
    /**
     * Tries to evaluate the function given the specified parameters, i.e. calculate the hyperbolic tangent of the
     * parameter.
        ...
        if(parameters.length==1){
            ...
            if(!value.hasFunction()){
                if(value.getMeasure()!=null){
                    newExpression = new ExpressionImpl(nl.wur.fbr.om.math.Math.tanh(value.getMeasure()));
                }else if(value.hasNumericalValue()){
                    newExpression = new ExpressionImpl(Math.tanh(value.getNumericalValue()));
                }else{
                    throw new MathException("Cannot calculate the hyperbolic tangent of "+value.getQuantity()+".");
                }
            } else newExpression =  new ExpressionImpl(this,value);
            return newExpression;
        }
        throw new MathException("The number of parameters for the hyperbolic tangent function is not equal to 1. (N="+parameters.length+")");
    }
     ...
     * The hyperbolic tangent always returns a dimensionless value.
     *
