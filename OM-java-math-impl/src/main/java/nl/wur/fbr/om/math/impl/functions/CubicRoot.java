...
/**
 * This is an implementation of the cubic root function.
 *
 * @author Don Willems on 08/10/15.
 */
public class CubicRoot implements Function {
    /**
     * Tries to evaluate the function given the specified parameters, i.e. calculate the cubic root of the parameter.
        ...
        if(parameters.length==1){
            ...
            if(!value.hasFunction()){
                if(value.getMeasure()!=null){
                    newExpression = new ExpressionImpl(nl.wur.fbr.om.math.Math.cbrt(value.getMeasure()));
                }else if(value.hasNumericalValue()){
                    newExpression = new ExpressionImpl(Math.cbrt(value.getNumericalValue()));
                }else{
                    throw new MathException("Cannot calculate the cubic root of "+value.getQuantity()+".");
                }
            } else newExpression =  new ExpressionImpl(this,value);
            return newExpression;
        }
        throw new MathException("The number of parameters for the cubic root function is not equal to 1. (N="+parameters.length+")");
    }
     ...
     * The dimension of the cubic root consists of the base dimensions of the parameter divided by 3.
     *
    ...
    public Dimension getDimensionOfResult(Expression... parameters) {
        ...
        for(BaseDimension bdim : bdims){
            dimension.setDimensionalExponent(bdim,pdim.getDimensionalExponent(bdim)/3.0);
        }
        return dimension;
    }
